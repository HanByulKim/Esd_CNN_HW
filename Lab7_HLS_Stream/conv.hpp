	...
	for(int filt=0; filt<NUM_FILTERS; filt++){
		for(int i=0; i<OUTPUT_SIZE; i++){
			for(int j=0; j<OUTPUT_SIZE; j++){
				#pragma HLS PIPELINE II=1
				out_stream[OUTPUT_SIZE*OUTPUT_SIZE*filt + OUTPUT_SIZE*i + j]=push_stream<w_t,U,TI,TD>(feature_map[filt][i][j], (OUTPUT_SIZE*OUTPUT_SIZE*filt + OUTPUT_SIZE*i + j) == (OUTPUT_SIZE*OUTPUT_SIZE*NUM_FILTERS-1));
			}
		}
	}
}
